...
class Model(nn.Module):
    def __init__(self, ifRender, device, if_4c, hand_model, use_mean_shape, pretrain, root_id=9, root_id_nimble=11, ifLight=True):
        ...
        elif hand_model == 'html':
            self.ncomps = [10, 48, 101] # shape, pose, no texture.
            self.hand_layer = MyHTMLLayer(ifRender, device, shape_ncomp=self.ncomps[0], pose_ncomp=self.ncomps[1], tex_ncomp=self.ncomps[2])
    ...
    def forward(self, dat_name, mode_train, images, Ks=None, root_xyz=None):
        ...
        elif self.hand_model == 'mano' or self.hand_model == 'html':
            # regress joints from verts
