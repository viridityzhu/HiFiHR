...
class Model(nn.Module):
    ...
    def forward(self, dat_name, mode_train, images, Ks=None, root_xyz=None):
        ...
        if self.hand_model == 'mano_new':
            ...
            # (bz, 25, 3)
            outputs['joints'] = outputs['nimble_joints'].clone()
            # Indices to remove along the second dimension
            joint_remove = [5, 10, 15, 20]
            joint_keep = [i for i in range(outputs['joints'].size(1)) if i not in joint_remove]
            
            # (bz, 21, 3)
            outputs['joints'] = torch.index_select(outputs['joints'], 1, torch.tensor(joint_keep).to(device))
