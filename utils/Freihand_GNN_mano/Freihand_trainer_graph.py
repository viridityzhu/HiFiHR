...
from dataloader.Freihand_dataloader_3d_rela import *
...
class dense_pose_Trainer(object):
    def __init__(self, train_data_loader, test_data_loader,mano_dir = '/mnt/data/allusers/haipeng/HandMethods/Freihand_GNNdemo/template',save_dir='./results'):
    ...
    def train(self, model, num_epochs):
        ...
        start_epoch = 1
        if os.path.exists(self.tmod_path):
            old_res = torch.load(self.tmod_path)
            model = old_res['model']
            start_epoch = len(old_res['train'])+1
            model.train_log = old_res['train']
            model.test_log = old_res['test']
            global_loss_vert = min(model.test_log)
            global_loss = min(model.test_log)
            ori_lr = lr
            lr = ori_lr*(0.1**int(start_epoch//50))
            if start_epoch > 1 and start_epoch % 50 == 0:
               lr = lr*10 
    ...
    def train_epoch(self, epoch, model, optimizer):
        ...
        for step, (batch_image, batch_pose_3d, batch_vertices,others) in enumerate(self.train_loader):
            ...
            vertex_loss = 0
            edge_length_loss_right = 0
            pose3d_loss = 0
            joint_pred = self.xyz_from_vertice(vertice_pred_list[-1]).permute(1,0,2)
            pose3d_loss = nn.L1Loss()(joint_pred, batch_pose_3d.cuda())
            # for idx in range(4):
            for idx in [0]:
                vertex_loss += nn.L1Loss()(vertice_pred_list[3-idx],batch_vertices_all[idx].cuda())
            ...
            total_loss = vertex_loss*1000.0 + edge_length_loss_right*100.0 + pose3d_loss*1000.0
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()
            if step % 10==0:
                print("Epoch", epoch, "|", "Step",step, "|",
                "Total_loss",total_loss.item(), "|","vertex loss", vertex_loss.item()*1000.0,
                "|","edg loss", edge_length_loss_right.item()*100.0,"|","pose loss", pose3d_loss.item()*1000.0)
            # update train_loss
